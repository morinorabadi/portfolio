	...
	return (
		<Card className="overflow-hidden border-slate-800 bg-slate-900/50 backdrop-blur">
			<CardContent className="p-0">
				<div className="grid gap-8 md:grid-cols-[1.5fr,1fr] lg:gap-12">
					{/* Video Section */}
					...
					<div className="flex flex-col justify-center space-y-6 p-6 md:p-8">
						...
						{project.features.length > 0 && (
							<div>
								<h4 className="mb-3 text-sm font-semibold uppercase tracking-wide text-tGreen">
									Key Features
								</h4>
								<ul className="space-y-2 text-sm text-slate-300">
									{project.features.map((feature, index) => (
										<li key={index} className="flex items-start">
											<span className="mr-2 mt-1.5 h-1.5 w-1.5 flex-shrink-0 rounded-full bg-tGreen" />
											<span>{feature}</span>
										</li>
									))}
								</ul>
